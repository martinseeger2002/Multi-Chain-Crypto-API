...
export function mintPadPepeUI() {
    ...
    function fetchAndDisplayCollections() {
        fetch('/api/v1/rc001pepe/collections')
            ...
            .then(data => {
                ...
                if (data.status === "success") {
                    ...
                    const createCollectionBox = ({ collectionName, collectionData }) => {
                        ...
                        fetch(`/api/v1/rc001pepe/collection/${collectionName}`)
                            .then(response => response.json())
                            .then(data => {
                                if (data.status === "success" && data.collection.length > 0) {
                                    ...
                                    inscriptionIframe.src = `https://pepinals.com/content/${randomInscription.inscription_id}`;
                        ...
                        if (collectionData.percent_minted < 100) {
                            ...
                            mintButton.addEventListener('click', () => {
                                fetch(`/api/v1/rc001pepe/mint_hex/${collectionName}`)
                                    .then(response => response.json())
                                    .then(mintData => {
